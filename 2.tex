...
\begin{figure}[ht]\centering
	...
	\pgfmathsetmacro{\parabolaTmpA}{sqrt(\h*\h + \rx*\rx)}
	\pgfmathsetmacro{\parabolaTmpYx}{\rx/\parabolaTmpA}
	\pgfmathsetmacro{\parabolaTmpYz}{\h/\parabolaTmpA}
	...
	\begin{scope}[canvas is plane={O(\parabolaTmpOx,\parabolaTmpOy,-\h)x(\parabolaTmpOx+\parabolaTmpXx,\parabolaTmpOy+\parabolaTmpXy,-\h)y(\parabolaTmpOx+\parabolaTmpYx,\parabolaTmpOy,-\h+\parabolaTmpYz)}]
		\pgfmathsetmacro{\xintercept}{\rx/\parabolaTmpXy};
		\pgfmathsetmacro{\yintercept}{\parabolaTmpA * (1 - \parabolatcos/2)/2};
		\pgfmathsetmacro{\parabolaCoeff}{-\yintercept/\xintercept/\xintercept};
		\draw[color=white,fill,pattern=north east lines,pattern color=black] plot[variable=\x,domain={-\xintercept}:{\xintercept},smooth] ({\x},{\parabolaCoeff*\x*\x+\yintercept}) -- cycle;
		\draw[mark position=0.5(parabolaC)] plot[variable=\x,domain={-\xintercept}:0,samples=20,smooth] ({\x},{\parabolaCoeff*\x*\x+\yintercept});
		\draw[variable=\x,domain=0:\xintercept,smooth,dashed] plot ({\x},{\parabolaCoeff*\x*\x+\yintercept});		
	...
	\begin{scope}[canvas is xz plane at y = 0]
		\draw (0,\h+1)--(0,-\h-1.5);
		\node[label = right:$\ell_1$] at (0,-\h-1.5) {};
